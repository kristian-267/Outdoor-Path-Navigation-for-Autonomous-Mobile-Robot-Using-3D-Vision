    ...
    def train(self):
        with EventStorage() as self.storage:
            ...
            for self.epoch in range(self.start_epoch, self.max_epoch):
                ...
                val_label_max = []
                for self.comm_info["iter"], self.comm_info["input_dict"] in self.data_iterator:
                    train_label_max.append(self.comm_info["input_dict"]['segment'].max())
                print(train_label_max)
                print(max(train_label_max))
                for self.comm_info["iter"], self.comm_info["input_dict"] in enumerate(self.val_loader):
                    val_label_max.append(self.comm_info["input_dict"]['segment'].max())
                print(val_label_max)
                print(max(val_label_max))
                time.sleep(3600)
