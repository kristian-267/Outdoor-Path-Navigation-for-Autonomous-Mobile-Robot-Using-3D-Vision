...
def calc_distance_heuristic(can_pass, gx, gy, ox, oy, resolution, rr):
    """
    ...
    while True:
        ...
        for i, _ in enumerate(motion):
            ...
            dtx = np.abs(can_pass[0] - x)
            dty = np.abs(can_pass[1] - y)
            dmx = np.abs(can_pass[2] - x)
            dmy = np.abs(can_pass[3] - y)
            dotx = np.abs(can_pass[4] - x)
            doty = np.abs(can_pass[5] - y)
            ...
            node = Node(current.x + motion[i][0],
                        ...
                        current.cost + w * motion[i][2], c_id)
