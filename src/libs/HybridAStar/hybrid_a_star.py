...
def get_neighbors(can_pass, current, config, ox, oy, kd_tree):
    for steer, d in calc_motion_inputs():
        node = calc_next_node(can_pass, current, steer, d, config, ox, oy, kd_tree)
        if node and verify_index(node, config):
            yield node


def calc_next_node(can_pass, current, steer, direction, config, ox, oy, kd_tree):
    x, y, yaw = current.x_list[-1], current.y_list[-1], current.yaw_list[-1]

    arc_l = MOTION_RESOLUTION
    ...
    # edge penalty
    dtx = np.abs(can_pass[0] - x_ind)
    dty = np.abs(can_pass[1] - y_ind)
    dmx = np.abs(can_pass[2] - x_ind)
    dmy = np.abs(can_pass[3] - y_ind)
    dotx = np.abs(can_pass[4] - x_ind)
    doty = np.abs(can_pass[5] - y_ind)
    if np.sum((dotx < 1) * (doty < 1)) > 0:
        w = 10.0
    elif np.sum((dmx < 1) * (dmy < 1)) > 0:
        w = 1.0
    elif np.sum((dtx < 1) * (dty < 1)) > 0:
        w = 0.1
    else:
        w = 100.0
    ...
    cost = current.cost + added_cost + w * arc_l
...
def analytic_expansion(can_pass, current, goal, ox, oy, kd_tree):
    ...
    for path in paths:
        if check_car_collision(path.x, path.y, path.yaw, ox, oy, kd_tree):
            cost = calc_rs_path_cost(can_pass, path)
...
def update_node_with_analytic_expansion(can_pass, current, goal,
                                        c, ox, oy, kd_tree):
    path = analytic_expansion(can_pass, current, goal, ox, oy, kd_tree)
    ...
    if path:
        ...
        f_cost = current.cost + calc_rs_path_cost(can_pass, path)
...
def calc_rs_path_cost(can_pass, reed_shepp_path):
    cost = 0.0
    ...
    sum_t = 0
    sum_bl = 0
    for i in range(1, len(reed_shepp_path.x)):
        x_ind = round(reed_shepp_path.x[i] / XY_GRID_RESOLUTION)
        y_ind = round(reed_shepp_path.y[i] / XY_GRID_RESOLUTION)
        dtx = np.abs(can_pass[0] - x_ind)
        dty = np.abs(can_pass[1] - y_ind)
        dmx = np.abs(can_pass[2] - x_ind)
        dmy = np.abs(can_pass[3] - y_ind)
    ...
    w = (10 * sum_ot + sum_m + 0.1 * sum_t + 100 * sum_bl) / (sum_ot + sum_m + sum_t + sum_bl)
...
def hybrid_a_star_planning(can_pass, start, goal, ox, oy, xy_resolution, yaw_resolution):
    """
    start: start node
    goal: goal node
    ...
    h_dp = calc_distance_heuristic(
        can_pass, goal_node.x_list[-1], goal_node.y_list[-1],
    ...
    while iter <= 100:
        ...
        if iter % 5 == 0:
            is_updated, final_path = update_node_with_analytic_expansion(
                can_pass, current, goal_node, config, ox, oy, obstacle_kd_tree)

            if is_updated:
                break

        for neighbor in get_neighbors(can_pass, current, config, ox, oy,
                                      obstacle_kd_tree):
...
def verify_index(node, c):
    x_ind, y_ind, yaw_ind = node.x_index, node.y_index, node.yaw_index
    if c.min_x <= x_ind <= c.max_x and c.min_y <= y_ind <= c.max_y and c.min_yaw <= yaw_ind <= c.max_yaw:
        return True
...
'''
def hybrid_a_star(ox, oy):
    print("Start Hybrid A* planning")

    ox, oy = [], []

    for i in np.arange(2, 12, 0.3):
        ox.append(i)
        oy.append(0.0)
    for i in np.arange(2, 12, 0.3):
        ox.append(6.0)
        oy.append(i)
    for i in np.arange(2, 12.2, 0.3):
        ox.append(i)
        oy.append(6.0)
    for i in np.arange(2, 12.2, 0.3):
        ox.append(0.0)
        oy.append(i)
    for i in np.arange(2, 8, 0.3):
        ox.append(2.0)
        oy.append(i)
    for i in np.arange(2, 8, 0.3):
        ox.append(4.0)
