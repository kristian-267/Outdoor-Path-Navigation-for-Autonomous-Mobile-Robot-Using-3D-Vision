...
class DTU_TrailDataset(Dataset):
    def __init__(self,
                 split="train",
                 ...
                 cache=False,
                 loop=1):
        super(DTU_TrailDataset, self).__init__()
    ...
    def get_data(self, idx):
        ...
        if not self.cache:
            ...
            cache_name = "thesis" + data_name.replace(os.path.sep, "-")
            data = shared_dict(cache_name)
        name = os.path.basename(self.data_list[idx % len(self.data_list)]).split("_")[0]
        coord = data["coord"]
        color = data["color"]
        scene_id = data_path
